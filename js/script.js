document.addEventListener("DOMContentLoaded", function() {
    ...
    const speechEl = document.getElementById('alien-speech');
    ...
    alienThumbs.forEach((thumb) => {
        thumb.addEventListener("click", () => {
            alienImg.src = thumb.src;
            speechEl.textContent = "Test!";
            if (audioUnlocked && audioPlayer) audioPlayer.play().catch(() => {});
        });
    });
});
...
const alienSoundMap = {
    'clara.png': 'sounds/clara.mp3',
    'henry.png': 'sounds/henry.mp3',
    'mama.png': 'sounds/mama.mp3',
    'olaf.png': 'sounds/olaf.mp3',
    'papa.png': 'sounds/papa.mp3'
};

let audioUnlocked = false;
let audioPlayer = null;

// User-Klick einmalig, um Audio freizuschalten
window.addEventListener('click', () => {
    if (!audioUnlocked) {
        audioUnlocked = true;
        audioPlayer = new Audio();
...
// Funktion, um den Sound zu spielen
function playAlienSound(src) {
    if (!audioUnlocked || !audioPlayer || !src) return;
    audioPlayer.src = src;
    audioPlayer.currentTime = 0;
    audioPlayer.play().catch(err => console.log('Audio-Fehler:', err));
}
...
function showAlienSpeech() {
    if (!speechEl || !alienImg) return;

    const name = getAlienBasename();
    const lines = alienSpeechMap[name];
    ...
    // --- SOUND ---
const soundSrc = alienSoundMap[name];
if (soundSrc) playAlienSound(soundSrc);
...
const backgroundAccentMap = {
    'backgrounds/magentaback.png': '#d520ab',
    'backgrounds/cyanback.png': '#1ecad3',
    'backgrounds/orangeback.png': '#ff8c1a'
};
...
bgButtons.forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        // Accent Color setzen
        const accent = backgroundAccentMap[bgPath];
        if (accent) {
